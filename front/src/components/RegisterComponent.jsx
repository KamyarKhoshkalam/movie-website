...
const RegisterComponent = () => {
  ...
  const handleSubmit = async (email, password, username, password2, e) => {
    e.preventDefault()
    ...
    if (parseInt(userCapcha) === capcha) {
      ...
      try {
        setLoading(true)
        const res = await privateApi.post('register/', {
          username,
          email,
          password,
          password2,
  ...
  return (
    <div className="flex h-screen w-screen items-center justify-between">
      ...
      <form
        ...
        onSubmit={(e) => {
          handleSubmit(email, password, username, password2, e)
        }}
      >
        <h2 className="text-2xl">Register to YekMovies</h2>
